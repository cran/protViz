...
\code{protViz} also provides and perl script \code{mascotDat2RData.pl} taking mascot server dat files as input and producing RData output.

\begin{verbatim}
$ /usr/local/lib/R/site-library/protViz/exec/mascotDat2RData.pl \
...
\bibitem{pPTM} Nanni, P., Panse, C., Gehrig, P., Mueller, S., Grossmann, J., Schlapbach, R., PTM MarkerFinder, a software tool to detect and validate spectra from peptides carrying post-translational modifications.  submitted to PROTEOMICS, 2013 (pmic.201300036.R1).
\end{thebibliography}
